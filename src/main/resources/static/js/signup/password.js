function checkPassword() {
    var idJ = /^(?=.*[A-Za-z])(?=.*\d)(?=.*[@$!%*#?&])[A-Za-z\d@$!%*#?&]{2,16}$/;

    document.getElementById('check2').innerHTML='ㅎㅇㅎㅇ'
    if(idJ.test(document.getElementById('mbr_password').value) === true) {
        document.getElementById('check2').innerHTML='사용가능한 비밀번호입니다.'
        document.getElementById('check2').style.color='#57a9ef';
    } else {
        document.getElementById('check2').innerHTML='2~16자 영어, 숫자, 특수문자를 포함하세요.';
...
function checkPassword2() {
    if(document.getElementById('mbr_password').value !=='' && document.getElementById('mbr_password2').value!=='') {
        if(document.getElementById('mbr_password').value === document.getElementById('mbr_password2').value) {
